...
import { GET_ALL_MESSAGES_ROUTE, GET_CHANNEL_MESSAGES_ROUTE, HOST } from "@/utils/contanst"
...
function MessageContainer() {
  ...
  useEffect(() => {
    ...
    const getMessages = async () => {
      try {
        const { data } = await apiClient.post(GET_ALL_MESSAGES_ROUTE, { id: selectedChatData._id }, { withCredentials: true })
        if (data.success) {
    ...
    const getChannelMessages=async ()=>{
      try {
        const { data } = await apiClient.get(`${GET_CHANNEL_MESSAGES_ROUTE}/${selectedChatData._id}`, { withCredentials: true })
        if (data.success) {
          setSelectedChatMessages(data.messages)
